...
export default function HistoryPage() {
  ...
  const [selected, setSelected]   = useState(null) // date string for drill-down
  ...
  function GoalStatus({ kcal }) {
    ...
    const pct  = Math.min(100, Math.round((kcal / calorieTarget) * 100))
  ...
  return (
    <AppLayout title={selected ? `← ${selected}` : 'History 📊'}>
      <div className="px-4 py-4">

        {/* Drill-down view */}
        {selected && selectedDay && (
          <>
            <button onClick={() => setSelected(null)} className="text-sm text-blue-600 font-semibold mb-4 flex items-center gap-1">
              ← Back to history
            </button>
            <div className="card p-4 mb-4">
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-3xl font-black text-slate-800">{selectedDay.kcal.toLocaleString()}</p>
                  <p className="text-xs text-slate-500">kcal · {selectedDay.count} meals</p>
                </div>
                <GoalStatus kcal={selectedDay.kcal} />
              </div>
              <div className="h-1.5 rounded-full bg-slate-100 mb-1">
                <div className={clsx('h-1.5 rounded-full', selectedDay.kcal <= calorieTarget ? 'bg-green-500' : 'bg-red-400')}
              ...
              <div className="flex gap-2 mt-3">
                <div className="flex-1 bg-slate-50 rounded-lg p-2 text-center">
                  ...
                  <p className="text-sm font-bold">{Math.round(selectedDay.protein)}g</p>
                </div>
                <div className="flex-1 bg-slate-50 rounded-lg p-2 text-center">
                  <p className="text-xs text-slate-400">Carbs</p>
                  <p className="text-sm font-bold">{Math.round(selectedDay.carbs)}g</p>
                </div>
                <div className="flex-1 bg-slate-50 rounded-lg p-2 text-center">
                  <p className="text-xs text-slate-400">Fat</p>
                  <p className="text-sm font-bold">{Math.round(selectedDay.fat)}g</p>
                </div>
              </div>
            </div>

            <div className="card p-4">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Meals</p>
                  ...
                  return (
                    <button key={day.date} onClick={() => setSelected(day.date)}
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">{day.count} meals logged</p>
                        <div className="h-1 rounded-full bg-slate-100 mt-2">
                          <div className={clsx('h-1 rounded-full', isOver ? 'bg-red-400' : 'bg-green-500')}
                            style={{ width: `${pct}%` }} />
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-base font-black text-blue-600">{day.kcal.toLocaleString()}</p>
